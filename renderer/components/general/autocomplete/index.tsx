...
export default function Autocomplete({ className, setOpen }: AutocompleteInterface ){
  ...
  return(
    <div ref={wrapperRef} className={"flex flex-col " + className}>
      ...
      <input maxLength={7} className="w-20 p-2 uppercase bg-transparent border border-solid select-none border-text text-text placeholder:text-highlight/50" value={search} onChange={handleChange} onClick={() => {
        if(search.length < 7){
          setDisplay(true)
        }
